...
BEGIN
	IF @CODIGO IN (SELECT Codigo FROM P_Nacional)
		SELECT N.Codigo, Tipo='Nacional', N.Direccion, N.Peso_gramos AS Peso, C.Nombre AS Conductor, R.Descripcion AS Ruta, RCC.Placa AS [Placa del Camion],RCC.Fecha AS Fecha FROM P_Nacional N
